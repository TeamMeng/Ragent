...
impl CalculatorTool {
    ...
    pub async fn call(&self, args: CalculatorInput) -> Result<CalculatorOutput, Box<dyn std::error::Error + Send + Sync>> {
...
fn parse_add_sub(t: &[Tok], i: usize) -> Result<(f64, usize), Box<dyn std::error::Error + Send + Sync>> {
    let (mut left, mut i) = parse_mul_div(t, i)?;
    while i < t.len() {
        match t[i] {
            Tok::Plus => { i += 1; let (r, p) = parse_mul_div(t, i)?; left += r; i = p; }
            Tok::Minus => { i += 1; let (r, p) = parse_mul_div(t, i)?; left -= r; i = p; }
            _ => break,
        }
    }
    Ok((left, i))
...
fn parse_mul_div(t: &[Tok], i: usize) -> Result<(f64, usize), Box<dyn std::error::Error + Send + Sync>> {
    let (mut left, mut i) = parse_atom(t, i)?;
    while i < t.len() {
        match t[i] {
            Tok::Star => { i += 1; let (r, p) = parse_atom(t, i)?; left *= r; i = p; }
            Tok::Slash => { i += 1; let (r, p) = parse_atom(t, i)?; if r == 0.0 { return Err("Division by zero".into()); } left /= r; i = p; }
            _ => break,
        }
    }
    Ok((left, i))
...
fn parse_atom(t: &[Tok], i: usize) -> Result<(f64, usize), Box<dyn std::error::Error + Send + Sync>> {
    if i >= t.len() { return Err("Unexpected end".into()); }
    match &t[i] {
        Tok::Num(n) => Ok((*n, i + 1)),
        Tok::LParen => { let (r, p) = parse_add_sub(t, i + 1)?; if p >= t.len() || t[p] != Tok::RParen { return Err("Missing )".into()); } Ok((r, p + 1)) }
        Tok::Minus => { let (r, p) = parse_atom(t, i + 1)?; Ok((-r, p)) }
...
mod tests {
    ...
    async fn test_basic() {
        ...
        let out = tool.call(CalculatorInput { expression: "2 + 3 * 4".into() }).await.unwrap();
