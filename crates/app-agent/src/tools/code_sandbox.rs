...
impl CodeSandboxTool {
    ...
    pub async fn call(
        ...
        match lang.as_str() {
            "python" | "python3" | "py" => {
                self.exec_python(&args.code, args.stdin.as_deref(), timeout_secs)
                    .await
            }
            "javascript" | "js" | "node" => {
                self.exec_node(&args.code, args.stdin.as_deref(), timeout_secs)
                    .await
            }
            "rust" | "rs" => {
                self.exec_rust(&args.code, args.stdin.as_deref(), timeout_secs)
                    .await
            }
